...
echo "<h2>🔍 Debug Ingredients Table</h2>";
...
try {
    ...
    if ($tableExists > 0) {
        ...
        $columns = $pdo->query("SHOW COLUMNS FROM ingredients")->fetchAll(PDO::FETCH_ASSOC);
        ...
        $sample = $pdo->query("SELECT ingredient_id, ingredient_name, ingredient_status FROM ingredients LIMIT 5")->fetchAll(PDO::FETCH_ASSOC);
