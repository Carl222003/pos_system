...
try {
    ...
    // Get request details with ingredient information
    $stmt = $pdo->prepare("
        SELECT 
            sur.request_id,
            sur.ingredient_id,
    ...
    error_log("Database error in get_stock_update_request_details.php: " . $e->getMessage());
    echo json_encode(['success' => false, 'error' => 'Database error occurred. Please try again.']);
} catch (Exception $e) {
    error_log("Error in get_stock_update_request_details.php: " . $e->getMessage());
